<template><div><h1 id="vite相比webpack的优势" tabindex="-1"><a class="header-anchor" href="#vite相比webpack的优势" aria-hidden="true">#</a> Vite相比Webpack的优势</h1>
<p>引用<a href="https://cn.vitejs.dev/guide/why.html#the-problems" target="_blank" rel="noopener noreferrer">Vite官方文档<ExternalLinkIcon/></a>中的一段话:</p>
<blockquote>
...
<!-- more -->
<h2 id="造成这种现象的直接原因" tabindex="-1"><a class="header-anchor" href="#造成这种现象的直接原因" aria-hidden="true">#</a> 造成这种现象的直接原因</h2>
<p>首先，我们要清除地了解到<code v-pre>Vite</code>相比<code v-pre>Webpack</code>有一点区别在于<code v-pre>Webpack</code>支持多种模块导入方式包括CommonJs 、 ES Modeule等等。</p>
<p>在启动时，它必须把所有的模块都打包构建，这样才能保证将所有的模块导入方式都转换为浏览器可以识别的模块导入方式。</p>
<p>由于<code v-pre>webpack dev server</code> 在启动时需要先<code v-pre>build</code>一遍，而这个过程需要消耗很多时间</p>
<figure><img src="@source/文章/分类/前端工程化/Vite/assets/image-20230812211327146.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>而Vite 不同的是 执行<code v-pre>vite serve</code> 时，内部直接启动了web Server, 并不会先编译所有的代码文件。</p>
<figure><img src="@source/文章/分类/前端工程化/Vite/assets/image-20230812211357219.png" alt="image-20230812211357219" tabindex="0" loading="lazy"><figcaption>image-20230812211357219</figcaption></figure>
<p>但是webpack 这类工具的做法是将所有模块提前编译、打包进bundle里，换句话说，不管模块是否会被执行，都要被编译和打包到bundle里。随着项目越来越大，打包后的bundle也越来越大，打包的速度自然会越来越慢。</p>
<h2 id="webpack可以优化吗" tabindex="-1"><a class="header-anchor" href="#webpack可以优化吗" aria-hidden="true">#</a> Webpack可以优化吗</h2>
<p>答案是不能，顶多我们按需引入，但是任然没有从根本上解决它导入无关模块这一个问题。所以如果一旦要改那么将会动到webpack的命脉。</p>
<p>因为webpack之所以强大，就是因为它强大的生态，因为它支持多种模块化:，所以你的工程将不只是跑在浏览器端。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>webpack的编译原理：使用 AST 抽象语法分析的工具，因为构建工具是运行在服务端的，所以他可以直接分析出你写的这个js文件有哪些导入和导出操作。</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 转换的核心代码（利用一个立即执行函数）</span>
...
<p><strong>综上：vite也不可能直接把webpack替代, vite是基于es modules的, 侧重点不一样, webpack更多的<code v-pre>关注兼容性</code>, 而vite关注<code v-pre>浏览器端的开发体验</code></strong></p>
</div></template>
